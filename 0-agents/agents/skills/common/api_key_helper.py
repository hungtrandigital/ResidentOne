...
3. 0-agents/agents/skills/.env (shared skills config)
4. Skill directory .env file
...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    """
    Find GEMINI_API_KEY using 4-step lookup:
    1. Process environment
    2. Project root .env
    3. 0-agents/agents/skills/.env (shared skills config)
    4. Skill directory .env
    ...
    # Step 3: Check 0-agents/agents/skills/.env (shared skills config)
    skills_env = project_dir / '0-agents' / 'agents' / 'skills' / '.env'
    if skills_env.exists():
        api_key = load_env_file(skills_env)
        if api_key:
            print(f"✓ Using API key from {skills_env}", file=sys.stderr)
            return api_key
...
def find_env_var(var_name: str, skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    # Step 2-4: Check .env files in order
    skills_dir = skill_dir.parent  # 0-agents/agents/skills
    env_files = [
        project_dir / '.env',                    # Project root
        skills_dir / '.env',                     # 0-agents/agents/skills/.env
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3️⃣  0-agents/agents/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/0-agents/agents/skills/.env", file=sys.stderr)

        print("\n4️⃣  Skill directory .env file:", file=sys.stderr)
