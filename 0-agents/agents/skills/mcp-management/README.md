...
```bash
cd 0-agents/agents/skills/mcp-management/scripts
npm install
```
...
See `0-agents/agents/skills/mcp-management/.mcp.json.example` for more examples.
...
- Load config from `.mcp.json` (project root)
...
2. `0-agents/agents/skills/mcp-management/.env`
3. `0-agents/agents/skills/.env`
4. `.env` (project root)
...
The `mcp-manager` agent (if exists in project) uses this skill to:
...
Ensure `.mcp.json` (project root) exists and is valid JSON.
