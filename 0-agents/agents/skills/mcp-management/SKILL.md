...
MCP servers configured in `.mcp.json` (project root) or `0-agents/agents/skills/mcp-management/.mcp.json`.
...
# Or if using skill-specific config:
mkdir -p .gemini && ln -sf 0-agents/agents/skills/mcp-management/.mcp.json .gemini/settings.json
```
...
- Config loading from `.mcp.json` (project root)
...
**Method 2: Scripts**
```bash
cd 0-agents/agents/skills/mcp-management/scripts && npm install
