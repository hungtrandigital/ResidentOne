...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.mcp.json` (project root):
...
ln -sf .mcp.json .gemini/settings.json

# Create symlink (Windows - requires admin or developer mode)
mklink .gemini\settings.json .mcp.json
```
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.mcp.json`)
...
Should show symlink pointing to `.mcp.json`.
