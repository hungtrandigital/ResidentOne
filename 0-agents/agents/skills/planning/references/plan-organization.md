...
**State File:** `<WORKING-DIR>/active-plan`
...
if [ -f "<WORKING-DIR>/active-plan" ]; then
  ACTIVE=$(cat <WORKING-DIR>/active-plan)
  if [ -d "$ACTIVE" ]; then
...
**Report Output Rules:**
1. Read `<WORKING-DIR>/active-plan` to get plan path
...
- **MANDATORY:** Success metrics and KPIs
- Validation methods
